...
export default function DealerDetail() {
  ...
  useEffect(() => {
    dealersApi.getByCode(code).then(setDealer);
  }, [code]);
  ...
  useEffect(() => {
    setInvoices(null);
    invoicesApi.list(`dealer=${code}`).then(setInvoices);
  }, [code]);
  ...
  return (
    <div>
      ...
      <div className="ph"><h3>Delivery challans / dispatches</h3><p>{totalCartons} carton(s) dispatched in total (excluding cancelled).</p></div>
      {invoices === null ? (
            ...
            <tbody>
              {invoices.map((i) => (
                <tr key={i.no}>
                  ...
                  <td>{i.createdBy?.name || i.bookedBy || i.by}</td>
