...
export default function GenerateBarcodes() {
  ...
  // undo a mistaken batch. If it refuses specifically because cartons are
  // dispatched (409), a second, separately-worded confirm offers to force
  // it through anyway — for test/junk batches that genuinely need their QR
  // history gone even though some cartons went out on a real dispatch. The
  // dispatch/invoice itself is never touched either way — only the QR
  ...
  async function deleteBatchRow(b) {
    ...
    try {
      ...
      setDeletingBatch(null);
      if (err.status === 409 && user.role === 'masterAdmin') {
        if (confirm(`${err.message}\n\nForce delete anyway? This removes the QR/tracking records only — the dispatch(es), invoice(s), and customer's received count are NOT touched or reversed. This cannot be undone.`)) {
          setDeletingBatch(b.batchId);
          try {
            const res2 = await barcodeApi.deleteBatch(b.batchId, { force: true });
            showToast(res2.message, 'g');
            loadRecentBatches(recentLimit);
            if (trackProduct) loadTrack(trackProduct);
            if (openBatch === b.batchId) { setOpenBatch(null); setOpenBatchDetail(null); }
          } catch (err2) { showToast(err2.message, 'err'); }
