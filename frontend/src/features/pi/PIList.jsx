  ...
  function buildCustomerGroups() {
    ...
    return Object.values(groups)
      .map((g) => ({
        ...
        latest: Math.max(...g.pis.map((p) => new Date(p.confirmedAt || p.createdAt).getTime())),
      }))
      ...
      {pis === null ? (
        ...
        <>
          <div className="tblwrap">
            <table className="dt">
              <thead><tr><th>PI no</th><th>Dealer</th><th>Assigned to</th><th>Items</th><th>Total ₹</th><th>Status</th><th>Created by</th><th>Created</th><th>Confirmed</th><th></th></tr></thead>
              <tbody>
                {pis.map((p) => {
                ...
                {!pis.length && <tr><td colSpan={10}><div className="empty">No PIs match</div></td></tr>}
              </tbody>
            </table>
          </div>
                  ...
                  <div className="card" key={g.code} style={{ marginBottom: 10 }}>
                    ...
                    {isOpen && (
                      <div className="tblwrap" style={{ marginTop: 10 }}>
                        <table className="dt">
                          <thead><tr><th>PI no</th><th>Items</th><th>Total ₹</th><th>Status</th><th>Created by</th><th>Created</th><th>Confirmed</th></tr></thead>
                          <tbody>
                            {g.pis.map((p) => (
                              <tr key={p.no}>
                                <td><Link to={`/pis/${p.no}`} className="mono"><b>{p.no}</b></Link></td>
